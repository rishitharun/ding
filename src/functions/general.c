...
static short nullifyChecksum(PacketObject* packet)
{
  short packet_type = packet->packet_type;

  switch(packet_type)
  {
    case __IP4:
      clearMemory( ((struct ip4_head*) (packet->head))->header_chksum, 2);
      return packet->head_len;

    case __ICMP:
      clearMemory(((struct icmp_head*)(packet->head))->checksum, 2);
...
UserVariable* sendfunc(Node* args)
{
  ...
  while(args != NULL)
  {
    ...
    PacketObject* packet = (PacketObject*) packet_variable->value;
...
UserVariable* container(Node* args)
{
  ...
  else { fprintf(stderr, ERROR"Please pass packet name as string argument <%d>! \n"RESET, val->datatype); return NULL; }
    ...
    *(return_var->value) += 1;
    ...
    switch(value->datatype)
    {
      ...
      case PACKET_TYPE_CODE:;
        PacketObject* temp = (PacketObject*) value->value;
...
UserVariable* checksum(Node* args)
{
  ...
  PacketObject* packet = (PacketObject*) (value->value);
  unsigned char* assmbld_packet = packet->assmbld_packet_struct;
