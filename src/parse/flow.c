...
boolean parseFlow(char *line, short line_len)
{
  ...
  /*INITIALIZATION OF PARSING ENVIRONMENT */
  Node* line_tree;
  Stack* line_stack;
  ...
  *index = buffer_ind = flag = allocd = cstr_offset = cstr_buff_ind = 0;
  ...
  /*END  ---  INITIALIZATION OF PARSING ENVIRONMENT */
  ...
  while(i<line_len)
  {
    ...
    switch(*(line+i))
    {
      ...
      case EQUAL_TOKEN:
        ...
        /* CONDITION STRING PARSING --- START */
        if(isCondStr)
        {
          if(!null_node)
          {
